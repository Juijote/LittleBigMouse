...
public class MainService : ReactiveModel, IMainService
{
    ...
    public async Task StartNotifierAsync()
    {
        ...
        //await _notify.AddMenuAsync(-1, "检查更新","Icon/lbm_on", CheckUpdateAsync);
        await _notify.AddMenuAsync(-1, "汉化更新","Icon/lbm_on", Juij);
        await _notify.AddMenuAsync(-1, "打开","Icon/lbm_off", ShowControlAsync);
        await _notify.AddMenuAsync(-1, "开始","Icon/Start", StartAsync);
        await _notify.AddMenuAsync(-1, "停止","Icon/Stop", () =>
        {
            MonitorsLayout.Enabled = false; 
            MonitorsLayout.SaveEnabled();
            return _littleBigMouseClientService.StopAsync();
        });
        await _notify.AddMenuAsync(-1, "退出", "Icon/sys/Close", QuitAsync);
    ...
    void OpenWebsite()
    {
        string websiteUrl = "https://juij.eu.org/#LittleBigMouse";
        ...
        {
            Console.WriteLine($"打开网址时出现异常：{ex.Message}");
        }
    }
